...
Авторизации пока нет, а приложение ещё не подключено к базе. Поэтому кнопки входа ведут
на заглушку `/api/auth/sign-in`, которая возвращает на `/login` с ошибкой «Вход пока
недоступен», а личная страница показывает только демо-профиль по адресу `/demo`.
...
| БД, авторизация, файлы | Supabase, Supabase CLI (миграции и типы) | схема ✅, подключение — шаг 7 |
...
в `.env` и заполни. Этот файл читают и Next.js, и Supabase CLI. Все переменные серверные
...
└─ shared/
   ...
   ├─ api/             # apiClient, ApiError, QueryClient; supabase/ — типы БД

...
- серверное подключение к Supabase и проверка сессии в API: `/api/me` отвечает гостю 401.

Входа пока нет, а профили ещё не читаются из базы. Поэтому кнопки входа ведут на заглушку
...
| БД, авторизация, файлы | Supabase (`@supabase/ssr`, `@supabase/supabase-js`), Supabase CLI (миграции и типы) | схема и серверные клиенты ✅, вход — шаги 8–10 |
...
сайт открывается, но API-роуты, которые работают с Supabase (сейчас `/api/me`), отвечают 500.
...
| GET | `/api/me` | текущий пользователь | частично: гостю 401, пользователю `{ user }` |
...
- Supabase (БД, авторизация, хранилище фото): см. ниже, подключение к приложению — шаг 7;

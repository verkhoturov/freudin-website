...
недоступен» (шаги 9–10). Онбординг и настройки — заглушки (шаги 13 и 15), поэтому после первого
входа пользователь попадает на пустую страницу `/onboarding`: API для создания профиля уже есть,
формы для него ещё нет. Пример личной страницы — демо-профиль по адресу `/demo`.

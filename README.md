...
недоступен» (шаги 9–10). Онбординг и настройки — заглушки, поэтому после первого входа
пользователь попадает на пустую страницу `/onboarding`. Личная страница показывает только
демо-профиль по адресу `/demo`.
...
| `/onboarding` | создание страницы после первого входа | заглушка, доступна только вошедшим без профиля |
| `/settings` | настройки профиля и аккаунта | заглушка, доступна только вошедшим с профилем |
...
| GET | `/api/me` | текущий пользователь, его профиль и подсказки для онбординга | готово |

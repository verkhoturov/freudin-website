...
входа). ✅ Vercel подключён: для каждого PR собирается превью.
...
- [x] Домен `freud.in` в `siteConfig`, `metadataBase`, Open Graph и карточка Twitter по умолчанию.
...
- OG-превью ссылок на личные страницы (`generateMetadata` + `opengraph-image`) и личные страницы
  в `sitemap.xml`. Мессенджеры и соцсети
...
| Шаг 5 | Проекты Supabase, Redirect URLs, пять переменных окружения (см. шаг 5) |

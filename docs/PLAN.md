...
| 13 | Юрисдикция | ✅ Пока считаем, что пользователи не из РФ: 152-ФЗ не учитываем, база во Франкфурте (eu-central-1). Юридические вопросы — отдельный этап H |
...
- [ ] Страницы `/privacy` и `/terms` с разделом «Удаление данных». Они нужны Meta для Live-режима
...
- [ ] Пройти загрузку, копирование фото провайдера и удаление с настоящей сессией (после шага 8).
...
- [ ] Пройти онбординг с настоящей сессией (после шага 8). Из облачной сессии сценарий проверен
      в браузере на прод-сборке с подменёнными ответами API.
...
- [ ] Пройти редактирование и удаление аккаунта с настоящей сессией (после шага 8). Из облачной
...
- [ ] Тексты `/privacy` и `/terms`, раздел «Удаление данных» с инструкцией для Meta.
      **Нужно от тебя:** оператор данных (ФИО, ИП или компания), контактный email и юрисдикция.
- [ ] Применимое право с учётом того, что база в ЕС (eu-central-1). Если появится аудитория
      из РФ, вернуться к 152-ФЗ (см. «Риски»).
...
| Этап H | Для `/privacy` и `/terms`: оператор данных, email и юрисдикция |

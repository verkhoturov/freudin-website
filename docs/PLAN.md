...
(этап G), `error.tsx` и доступность (шаг 17), главная для вошедшего (шаг 16), тесты (по запросу).
...
- [ ] (аудит) **Делаешь ты:** выключить провайдер Email в Supabase → Authentication →
      Sign In / Providers. Он не используется, но включён и открывает регистрацию в обход OAuth.
- [ ] (аудит) **Делаешь ты:** проверить план Supabase. На Free проект засыпает после недели без
...
- [ ] (аудит) Миграция: роль `anon` читает из `profiles` только публичные колонки (`username`,
      ...
      publishable key в браузер не попадает.
...
- [ ] (аудит) Проверка заголовка `Origin` (или `Sec-Fetch-Site`) в изменяющих запросах API:
      вторая линия защиты от CSRF после cookies с `SameSite=Lax`.
...
- [ ] Пройти вход и выход через Google на localhost, затем на проде. Google в Supabase уже
      включён: аудит проверил, что `/auth/v1/authorize` ведёт на Google.
- [ ] (аудит) На `/login` показывать только подключённых провайдеров: сейчас кнопки Facebook
      и Telegram на проде ведут на «Способ входа недоступен». Список подключённых провайдеров
      держать в одном месте для клиента и сервера.
...
- [ ] (аудит) Ограничить размеры фото на сервере: прямым запросом к API в обход кропа можно
      ...
      или хотя бы проверять размеры по заголовку файла.
...
- [ ] (аудит) Если профиль уже создан (409 «Страница уже создана»: например, ответ на первую
      отправку не дошёл), перезапрашивать `/api/me`, чтобы гард увёл на страницу. Сейчас
      пользователь остаётся на онбординге до перезагрузки.
- [ ] (аудит) Показывать «Адрес свободен», когда проверка адреса прошла.
...
- [ ] (аудит) Баг: после ухода со страницы профиля заголовок вкладки становится «Freudin»
...
- [ ] (аудит) Кнопки соцсетей подписаны только платформой: две ссылки «Сайт» неотличимы,
      ника не видно. Показывать ник или домен: `Telegram · @anna`, `example.com`.
- [ ] (аудит) Адрес в другом регистре (`/Anna`) открывается, но не заменяется на `/anna`:
      дубли страницы и отдельные записи в кеше. Заменять адрес на канонический.
...
- [ ] (аудит) Баг: «Сохранить» без изменений показывает «Изменения сохранены», хотя запросов
      не было. Кнопка неактивна, пока в форме и фото нет изменений.
- [ ] (аудит) Предупреждать о несохранённых изменениях при уходе со страницы настроек.
- [ ] (аудит) Удаление аккаунта: если удаление пользователя упадёт после удаления файлов,
      в профиле останется ссылка на удалённое фото. Сначала обнулять `avatar_path` или удалять
      пользователя первым, а файлы — после.
...
- [ ] (аудит) **Нужно от тебя:** Vercel → Settings → Functions → регион `fra1`. Сейчас функции
      работают в `iad1` (Вашингтон), а база во Франкфурте: каждый запрос к базе пересекает
      Атлантику (порядка 90 мс).
...
- [ ] (аудит) Пока текстов нет, закрыть `/privacy` и `/terms` от индексации (`noindex`) и убрать
      их из `sitemap.xml`: сейчас пустые страницы индексируются.
...
| Шаг 5 (аудит) | Выключить провайдер Email в Supabase, проверить план и бэкапы |
| Шаг 8 | Google Cloud: OAuth-клиент и экран согласия — Google в Supabase уже включён, осталось проверить вход |
...
| Шаг 18 (аудит) | Регион функций Vercel `fra1`, защита ветки `main` в GitHub |
